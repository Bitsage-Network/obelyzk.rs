...
if [[ -z "$PROOF_MODE" ]]; then
    ...
    if [[ "$ENTRYPOINT_IN_PROOF" == "verify_model_gkr" || "$ENTRYPOINT_IN_PROOF" == "verify_model_gkr_v2" || "$ENTRYPOINT_IN_PROOF" == "verify_model_gkr_v3" || "$ENTRYPOINT_IN_PROOF" == "verify_model_gkr_v4" || "$ENTRYPOINT_IN_PROOF" == "verify_model_gkr_v4_packed" || "$ENTRYPOINT_IN_PROOF" == "verify_model_gkr_v4_packed_io" || "$ENTRYPOINT_IN_PROOF" == "verify_gkr_from_session" ]]; then
        PROOF_MODE="gkr"
...
" 2>/dev/null || echo "")
                ...
                if [[ "$ENTRYPOINT" != "verify_model_gkr" && "$ENTRYPOINT" != "verify_model_gkr_v2" && "$ENTRYPOINT" != "verify_model_gkr_v3" && "$ENTRYPOINT" != "verify_model_gkr_v4" && "$ENTRYPOINT" != "verify_model_gkr_v4_packed" && "$ENTRYPOINT" != "verify_model_gkr_v4_packed_io" ]]; then
                    UNSUPPORTED_REASON=$(parse_json_field "$PROOF_FILE" "verify_calldata.reason")
                    if [[ -z "$UNSUPPORTED_REASON" ]]; then
                        UNSUPPORTED_REASON=$(parse_json_field "$PROOF_FILE" "soundness_gate_error")
                    fi
                    WEIGHT_OPENING_MODE=$(parse_json_field "$PROOF_FILE" "weight_opening_mode")
                    err "verify_calldata.entrypoint must be verify_model_gkr / v2 / v3 / v4 / v4_packed / v4_packed_io in gkr mode (got: ${ENTRYPOINT})"
