...
| `direct` | **(Deprecated)** JSON with `batched_calldata` for legacy `verify_model_direct()` — removed from EloVerifier v11, use `ml_gkr` instead |
...
|----------|---------|---------|----------|
| **GKR (recommended)** | GPU prove (~103s full model) | — | `verify_model_gkr_v4()` (streaming) |
