    ...
    for &node_id in &topo {
        ...
        let output = match &node.op {
            ...
            GraphOp::Activation {
                ...
                // Use the same activation path as the prover (unreduced inputs
                // for GKR claim chaining). The old code masked inputs to table
                // range which produced different outputs than the prover.
